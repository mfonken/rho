...
//            LOG_RHO_CAP(DEBUG_RHO_CAP, "cap_x - %p > %p\n", capture_address, cap_x);
            index_t l = _->blobs[i].w;
            if( x + l > length )
                l = length - x;
            if( l <= 0 )
                continue;
//            LOG_RHO_CAP(DEBUG_RHO_CAP, "thesh - %p > ", thresh_address);
